...
#' posterior_bySufficientStatistics(obj = obj,ss=ss)
#' obj
#' ## weighted sample
#' posterior(obj=obj,ss=x,w=w)
#' obj$gamma$a
...
#' ss <- sufficientStatistics(obj = obj,x=x)
#' marginalLikelihood_bySufficientStatistics(obj=obj,ss = ss,LOG = TRUE) #marginal likelihood by sufficient statistics
...
dObservationDistribution.CatDirichlet <- function(obj,x,p,LOG=TRUE){
    if(missing(x)|missing(p)) stop("'x' and 'p' must be both specified")
    if(!is.vector(x)) x <- as.vector(x)
    if(length(p)!=length(obj$gamma$alpha))  stop("length of 'p' and Dirichlet parameters don't match")
    out <- p[match(x,obj$gamma$uniqueLabels)]
    if(LOG) out <- log(out)
    out

}

rObservationDistribution.CatDirichlet <- function(obj,n,p){
    if(missing(n)|missing(p)) stop("'n' and 'p' must be both specified")
    if(length(p)!=length(obj$gamma$alpha))  stop("length of 'p' and Dirichlet parameters don't match")
    n <- as.integer(n)
    obj$gamma$uniqueLabels[rCategorical(n=n,p=p)]
}
